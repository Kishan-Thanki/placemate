...
class CompanyDriveApplicationViewSet(BaseViewSet):
    ...
    def get_serializer_context(self):
        ...
        if self.request.user.is_authenticated:
            try:
                context['student_profile'] = StudentProfile.objects.get(user_id=self.request.user.id)
    ...
    def get_serializer_class(self):
        if self.action == 'create':
            ...
            return CompanyDriveApplicationBaseSerializer
    ...
    def get_queryset(self):
        ...
        if role in ['Admin', 'Student Placement Cell']:
            return CompanyDriveApplication.objects.all()\
                .select_related(
                    'student__user',
                    'company_drive__company',
                    'company_drive__placement_drive',
    ...
    def get_object(self):
        ...
        if (hasattr(self.request.user, 'active_role') and 
            self.request.user.active_role == 'Student' and
            obj.student.user_id != self.request.user.id):
    ...
    def accept_offer(self, request, pk=None):
        ...
        try:
            with transaction.atomic():
                ...
                # Find all other Applied applications of student
                other_applications = CompanyDriveApplication.objects.filter(
                    student=application.student,
                    status='Applied'
                ).exclude(id=application.id)
                
                # Delete Particular Student Other Applied Applications
                withdrawn_count = other_applications.count()
                other_applications.delete()
                ...
                return SuccessResponse(
                    message=f"Job offer accepted successfully. {withdrawn_count} other applications withdrawn."
                )
                
        except Exception as e:
            return ErrorResponse(message=f"An error occurred while accepting the offer: {str(e)}")
